...
- `scores`：每个玩家、来源、歌曲、难度的当前最佳状态，供 AI 和普通查询快速读取。
- `score_syncs`：每次同步一条批次记录，保存收到的记录数、当前谱面数和新增变化数。
- `score_history`：成绩状态变化历史，保存良、可、不可、连打、连击、游玩/通关/全连/全良次数、分数、评价、时间与原始 JSON。
- 历史用内容哈希去重；同步结果没有变化时仍记录批次，但不会复制整份成绩，避免数据库无意义膨胀。
- 以上表位于 AstrBot 插件数据目录，插件代码更新不会覆盖。
